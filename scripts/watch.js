(function () {
      ...
      return `
        <a class="video-link" href="watch.html?id=${encodeURIComponent(s.id)}">
          <div class="video-preview">
            <img class="thumbnail" src="${s.thumbnail}" alt="${
        s.thumbnailAlt || s.title
      }" loading="lazy" />
            <div>
  ...
  commentForm.addEventListener('submit', (e) => {
    ...
    renderComments();
    commentInput.value = '';
    if (typeof window.showToast === 'function') window.showToast('Comment posted');
  });
  ...
  subscribeButton.addEventListener('click', () => {
    ...
    if (typeof window.showToast === 'function')
      window.showToast(cur ? 'Unsubscribed' : 'Subscribed');
  });
  ...
  likeButton.addEventListener('click', () => {
    ...
    updateLikeUI();
    if (typeof window.showToast === 'function') window.showToast(cur ? 'Unliked' : 'Liked');
  });
  ...
  saveButton.addEventListener('click', () => {
    ...
    updateSaveUI();
    if (typeof window.showToast === 'function')
      window.showToast(cur ? 'Removed from saved' : 'Saved to Watch Later');
  });
})();
